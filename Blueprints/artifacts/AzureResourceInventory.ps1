...
$Global:SRuntime = Measure-Command -Expression {
    ...
    Function Extractor {
        ...
        function checkAzCli() {
            ...
            Write-Host 'Validating Az.Accounts Module..'
            $VarAz = Get-InstalledModule -Name Az -ErrorAction silentlycontinue
            Write-Debug ('Az.Accounts Module Version: ' + ([string]$VarAz.Version.Major + '.' + [string]$VarAz.Version.Minor + '.' + [string]$VarAz.Version.Build))
            if ($null -eq $VarAz) {
                Write-Host 'Trying to install Az.Accounts Module..'
                Install-Module -Name Az.Accounts -Force
            }
            $VarAz = Get-InstalledModule -Name Az.Accounts -ErrorAction silentlycontinue
            if ($null -eq $VarAz) {
    ...
    Function RunMain {
        ...
        #>
            ...
            #EndRegion            
            #Region Azure Resource Move Job
            <######################################################### Azure Resource Move Job ######################################################################>

            Write-Debug ('Starting Azure Resource Move Job.')
            Start-Job -Name 'ResourceMove' -ScriptBlock {

                If ($($args[5]) -eq $true) {
                    $ModuSeq = (New-Object System.Net.WebClient).DownloadString($($args[6]) + '/Extras/ResourceMove.ps1')
                }
                Else {
                    if ($($args[0]) -like '*\*') {
                        $ModuSeq0 = New-Object System.IO.StreamReader($($args[0]) + '\Extras\ResourceMove.ps1')
                    }
                    else {
                        $ModuSeq0 = New-Object System.IO.StreamReader($($args[0]) + '/Extras/ResourceMove.ps1')
                    }
                ...
                $ResMoveRun = ([PowerShell]::Create()).AddScript($ScriptBlock).AddArgument($null).AddArgument($($args[1])).AddArgument($null).AddArgument($($args[3])).AddArgument($($args[4]))

                $ResMoveJob = $ResMoveRun.BeginInvoke()

                while ($ResMoveJob.IsCompleted -contains $false) { Start-Sleep -Milliseconds 100 }

                $SubResult = $ResMoveRun.EndInvoke($ResMoveJob)

                $ResMoveRun.Dispose()

                $SubResult

            } -ArgumentList $PSScriptRoot, $Resource, $Resources, 'Processing' , $File, $RunOnline, $RawRepo | Out-Null

            #EndRegion   
            #Region Resource Group Job
            <######################################################### RESOURCE GROUP JOB ######################################################################>
            ...
            #Region Resources Loop Creation
            ...
            #Region Subscriptions Reporting
            <################################################################### SUBSCRIPTIONS ###################################################################>
            ...
            #Region ResourceMove Reporting
            <################################################################### ResourceMove ###################################################################>

            Write-Debug ('Generating Resource Move Evaluation sheet for: ' + $ResourceMove.count + ' ResourceMove.')

            Write-Progress -Activity 'Azure Resource Inventory ResourceMove1' -Status '50% Complete.' -PercentComplete 50 -CurrentOperation 'Building ResourceMove1 Sheet'

            If ($RunOnline) {
                Write-Debug ('Looking for the following file: ' + $RawRepo + '/Extras/ResourceMove.ps1')
                $ModuSeq = (New-Object System.Net.WebClient).DownloadString($RawRepo + '/Extras/ResourceMove.ps1')
            }
            Else {
                if ($PSScriptRoot -like '*\*') {
                    Write-Debug ('Looking for the following file: ' + $PSScriptRoot + '\Extras\ResourceMove.ps1')
                    $ModuSeq0 = New-Object System.IO.StreamReader($PSScriptRoot + '\Extras\ResourceMove.ps1')
                }
                else {
                    Write-Debug ('Looking for the following file: ' + $PSScriptRoot + '/Extras/ResourceMove.ps1')
                    $ModuSeq0 = New-Object System.IO.StreamReader($PSScriptRoot + '/Extras/ResourceMove.ps1')
            ...
            $ResMovRun = ([PowerShell]::Create()).AddScript($ScriptBlock).AddArgument($null).AddArgument($resource).AddArgument('Reporting').AddArgument($file).AddArgument($SmaResources).AddArgument($TableStyle)

            $ResMoveJob = $ResMovRun.BeginInvoke()

            while ($ResMoveJob.IsCompleted -contains $false) { Start-Sleep -Milliseconds 100 }

            $ResMovRun.EndInvoke($ResMoveJob)

            $ResMovRun.Dispose()

            Write-Progress -Activity 'Azure Resource Inventory Resource Move1 Evaluation' -Status '100% Complete.' -Completed
            #EndRegion
